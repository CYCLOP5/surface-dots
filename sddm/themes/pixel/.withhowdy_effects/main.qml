...
Item {
    ...
    readonly property bool enableFaceAuth: config.enableFaceAuth !== "false"
    ...
    Timer {
        ...
        onTriggered: root.authStatus = "idle"
    }

    // SUCCESS TIMER
    // Assumes Howdy works. Triggers "Verified" animation after 800ms
    Timer {
        id: fakeSuccessTimer
        interval: 900 
        repeat: false
        onTriggered: {
            if (passwordInput.text.length === 0 && root.authStatus === "scanning") {
    ...
    Connections {
        ...
        function onLoginFailed() {
            ...
            // Override fake success
            fakeSuccessTimer.stop()
        ...
        function onLoginSucceeded() {
            ...
            // Force verified in case fake timer hasn't fired yet
            root.authStatus = "verified"
        }
    }
    ...
    function wake() {
        if (!root.authOpen) {
            ...
            if (root.enableFaceAuth && passwordInput.text === "") {
                attemptLogin();
            } else {
    ...
    function attemptLogin() {
        ...
        // Start scanning + timer if no password entered
        if (passwordInput.text.length === 0 && root.enableFaceAuth) {
            root.authStatus = "scanning"
            fakeSuccessTimer.restart()
            safetyTimer.restart()
        } else {
            root.authStatus = "idle"
            fakeSuccessTimer.stop()
        ...
        loginDelayTimer.uname = uname;
        loginDelayTimer.pass = passwordInput.text;
        loginDelayTimer.idx = sessionIdx;
        loginDelayTimer.restart();
    }
    ...
    Timer {
        ...
        property int idx
        onTriggered: sddm.login(uname, pass, idx)
    }
    ...
    Component.onCompleted: {
        ...
        // Auto-trigger fake scan on load
        if (root.enableFaceAuth && passwordInput.text === "") {
             root.authStatus = "scanning"
             fakeSuccessTimer.restart()
        }
    ...
    TextInput {
        ...
        focus: !root.authOpen
        Keys.onPressed: (event) => { wake(); event.accepted = true; }
    }
    ...
    FaceUnlock {
        ...
        colorVerified: root.cGreen
        colorError: root.cError

        visible: root.enableFaceAuth
        opacity: root.authOpen ? 1.0 : 0.0
        Behavior on opacity { NumberAnimation { duration: 300 } }
    }
                ...
                Rectangle {
                    ...
                    Item {
                        ...
                        TextInput {
                            ...
                            // Kill Switch: User typing overrides fake success
                            onTextChanged: {
                                if (text.length > 0) {
                                    root.authStatus = "idle"
                                    fakeSuccessTimer.stop()
                            ...
                            Keys.onPressed: (event) => { 
                                if (event.key === Qt.Key_Down && root.sessionOpen) sessionList.forceActiveFocus() 
                            }
                        }
                        
                        Text {
                            anchors.centerIn: parent; 
                            text: (root.authStatus === "scanning") ? "Looking for you..." : "Enter Password"
                            color: root.cMuted
                            
                            visible: passwordInput.text.length === 0
                            font.pixelSize: 16
                            
                            opacity: (root.authStatus === "scanning" && visible) ? 0.5 : 1.0
